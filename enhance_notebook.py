...
def add_random_undersampling(nb):
    ...
    # Find the cell after BorderlineSMOTE
    smote_idx = find_cell_index(nb, 'BorderlineSMOTE')
    
    if smote_idx == -1:
        print("⚠️  BorderlineSMOTE not found, adding at end of sampling section")
    ...
    # Add markdown header
    markdown_cell = new_markdown_cell("""### 4.2.2 Random Undersampling
...
""")
    ...
    code_cell = new_code_cell("""# Import Random Undersampling
...
""")
    ...
    viz_cell = new_code_cell("""# Visualize comparison of sampling strategies
...
def add_feature_selection(nb):
    ...
    # Find appropriate location (after data preprocessing, before model training)
    prep_idx = find_cell_index(nb, 'X_train')
    ...
    # Add markdown header
    markdown_cell = new_markdown_cell("""## 5. Feature Selection
...
""")
    ...
    # Add Chi-square implementation
    chi2_cell = new_code_cell("""### 5.1 Chi-square Feature Selection
...
""")
    ...
    # Add RFE implementation
    rfe_cell = new_code_cell("""### 5.2 Recursive Feature Elimination (RFE)
...
""")
    ...
    comparison_cell = new_code_cell("""### 5.3 Feature Selection Comparison
...
# Venn diagram
venn2([chi2_set, rfe_set], 
      set_labels=('Chi-square', 'RFE'),
      ax=axes[0])
axes[0].set_title('Feature Selection Methods Overlap', fontsize=14, fontweight='bold')

# Feature importance comparison (top 15)
top_n = 15
chi2_top = chi2_scores.head(top_n).set_index('Feature')['Chi2_Score']
rfe_top = rfe_rankings.head(top_n).set_index('Feature')['RFE_Ranking']
...
# Plot
x = range(top_n)
axes[1].barh(x, chi2_top_norm.values, alpha=0.6, label='Chi-square (normalized)', color='#3498db')
axes[1].set_yticks(x)
axes[1].set_yticklabels(chi2_top.index, fontsize=9)
axes[1].set_xlabel('Normalized Score', fontsize=12)
axes[1].set_title(f'Top {top_n} Features by Chi-square', fontsize=14, fontweight='bold')
axes[1].legend()
axes[1].invert_yaxis()
...
""")
    ...
    # Add feature sets dictionary
    feature_sets_cell = new_code_cell("""### 5.4 Create Feature Sets for Model Training
...
""")
    ...
    # Find a good insertion point (look for model training section)
    ...
    # Insert all cells
    nb.cells.insert(insert_idx, markdown_cell)
    nb.cells.insert(insert_idx + 1, chi2_cell)
    nb.cells.insert(insert_idx + 2, rfe_cell)
    nb.cells.insert(insert_idx + 3, comparison_cell)
    nb.cells.insert(insert_idx + 4, feature_sets_cell)
...
def add_comprehensive_comparison(nb):
    ...
    # Add markdown header
    markdown_cell = new_markdown_cell("""## 6. Comprehensive Model Comparison
...
""")
    ...
    # Add comprehensive training code
    training_cell = new_code_cell("""### 6.1 Train All Model Configurations
...
""")
    ...
    results_cell = new_code_cell("""### 6.2 Results Analysis
...
""")
    ...
    viz_cell = new_code_cell("""### 6.3 Performance Visualizations
...
""")
    ...
    nb.cells.insert(insert_idx, markdown_cell)
    nb.cells.insert(insert_idx + 1, training_cell)
    nb.cells.insert(insert_idx + 2, results_cell)
    nb.cells.insert(insert_idx + 3, viz_cell)
